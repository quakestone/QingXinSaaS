...
import com.qingxinsaas.flowable.domain.dto.FlowCommentDto;
import com.qingxinsaas.flowable.domain.dto.FlowNextDto;
import com.qingxinsaas.flowable.domain.dto.FlowTaskDto;
...
import com.qingxinsaas.flowable.util.CustomProcessDiagramGenerator;
import com.qingxinsaas.flowable.util.FindNextNodeUtil;
import com.qingxinsaas.flowable.util.FlowableUtils;
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                if (StringUtils.isNotBlank(taskList.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    SysUser sysUser = getUserById(Long.parseLong(taskList.get(0).getAssignee()));
                ...
                if (StringUtils.isNotBlank(historicTaskInstance.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    SysUser sysUser = getUserById(Long.parseLong(historicTaskInstance.get(0).getAssignee()));
                    if (Objects.nonNull(sysUser)) {
                        flowTask.setAssigneeId(sysUser.getUserId());
                        flowTask.setAssigneeName(sysUser.getNickName());
                        flowTask.setAssigneeDeptName(Objects.nonNull(sysUser.getDept()) ? sysUser.getDept().getDeptName() : "");
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
    ...
    public AjaxResult revokeProcess(FlowTaskVo flowTaskVo) {
        Task task = taskService.createTaskQuery()
                ...
                .singleResult();
        if (task == null) {
            throw new FlowableException("流程未启动或已执行完成，无法撤回");
        }
        ...
        if (null == myTaskId) {
            throw new FlowableException("该任务非当前用户提交，无法撤回");
        }
...
//        TODO 传入名称查询不到数据?
            ...
            for (HistoricActivityInstance histIns : list) {
                ...
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        SysUser sysUser = getUserById(Long.parseLong(histIns.getAssignee()));
                        flowTask.setAssigneeId(sysUser.getUserId());
                        flowTask.setAssigneeName(sysUser.getNickName());
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        ...
                        if ("candidate".equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                SysUser sysUser = getUserById(Long.parseLong(identityLink.getUserId()));
                                stringBuilder.append(sysUser.getNickName()).append(",");
                            }
                            if (StringUtils.isNotBlank(identityLink.getGroupId())) {
                                SysRole sysRole = getRoleById(Long.parseLong(identityLink.getGroupId()));
    ...
    private SysUser getUserById(Long userId) {
        R<SysUser> result = remoteUserService.selectUserById(userId, SecurityConstants.INNER);
    ...
    private SysRole getRoleById(Long roleId) {
        R<SysRole> result = remoteRoleService.selectRoleById(roleId, SecurityConstants.INNER);
